...
void Aura::PeriodicTick()
{
    ...
    switch (m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_HEALTH_FUNNEL:
        {
            ...
            pdamage -= victim->GetResilienceRatingDamageReduction(pdamage, SpellDmgClass(spellProto->DmgClass), true);

            victim->CalculateDamageAbsorbAndResist(caster, GetSpellSchoolMask(spellProto), DOT, pdamage, &absorb, &resist, IsReflectableSpell(spellProto), IsResistableSpell(spellProto) && !spellProto->HasAttribute(SPELL_ATTR_EX5_NO_PARTIAL_RESISTS));
